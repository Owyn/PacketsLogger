...
module.exports = function PacketsLogger(mod) {
	...
	let logFile = fs.createWriteStream('tera-log.log', { flags: 'a' });
	let logC = false;
	let logS = false;
	let logRaw = true;
	...
	let blacklist = ['S_F2P_PremiumUser_Permission',
...
'C_PLAYER_LOCATION',
 ...
 'S_QUEST_BALLOON',
	...
	command.add('recreate',()=>{
		logFile=fs.createWriteStream('tera-log.log', { flags: 'a' });
	});
	...
	function enableHook(){
		...
		hook=mod.hook('*', 'raw', { order: 999999, filter: { fake: null, silenced: null, modified: null} }, (code, data, incoming, fake) => {
			if (!logC && !logS) return
